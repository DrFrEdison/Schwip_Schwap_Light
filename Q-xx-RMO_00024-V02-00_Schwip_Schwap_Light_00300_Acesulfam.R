...
dir( pattern = "Rsource" )
source.file <- print("Rsource_Schwip_Schwap_Light_mop_val_V02.R")
...
dt$para.pls$wlr <- wlr_function(200:300, 200:320, 5); nrow(dt$para.pls$wlr)
dt$para.pls$wlm <- wlr_function_multi(200:300, 200:320, 10); nrow(dt$para.pls$wlm)
...
setwd(wd$model[[ grep(dt$para$customer, names(wd$model)) ]])
fwrite(x = cbind(dt$model.raw$data, dt$model.raw$spc)
       , file = paste0( paste(dt$para$customer, dt$para$beverage, dt$para$substance[ dt$para$i ], sep = "_"), ".csv")
       , sep = ";", dec = ".", na = NA)
