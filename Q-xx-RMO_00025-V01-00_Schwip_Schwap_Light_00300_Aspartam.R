...
# PLS para####
dt$para.pls$wlr <- .wlr_function(200:250, 200:250, 5)
nrow(dt$para.pls$wlr)
dt$para.pls$wlm <- .wlr_function_multi(200:250, 200:250, 10)
...
dt$para.pls$wl[1,] <- c(200,235,NA,NA)
...
dt$pls$merge <- .merge_pls(pls_pred = dt$pls$pred, dt$pls$lm ,R2=.95
                           , mean = c(dt$para$SOLL[ dt$para$i ] * 90 / 100, dt$para$SOLL[ dt$para$i ] * 110 / 100))
head(dt$pls$merge[ order(dt$pls$merge$sd) , ], 20)
